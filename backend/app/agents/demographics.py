...
from app.prompts import DEMOGRAPHICS_PROMPT
from app.schemas import DemographicsOutput, ScenarioContext, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
from app.rag.retrieval_service import get_retrieval_service


async def run_demographics(
...
) -> tuple[DemographicsOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "demographics", n_results=2)
	...
	result = await call_agent(DEMOGRAPHICS_PROMPT, user_content)
	try:
		output = DemographicsOutput.model_validate({**result, "agent_name": "demographics"})
