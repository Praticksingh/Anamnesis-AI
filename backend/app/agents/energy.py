...
from app.prompts import ENERGY_PROMPT
from app.schemas import EnergyOutput, ScenarioContext, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
...
async def run_energy(
...
) -> tuple[EnergyOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	# Energy domain uses 'climate' RAG source set for volcanic/meteorological/resource grounding
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "climate", n_results=2)
	...
	result = await call_agent(ENERGY_PROMPT, user_content)
	try:
		output = EnergyOutput.model_validate({**result, "agent_name": "energy"})
