...
from app.prompts import HEALTHCARE_PROMPT
from app.schemas import HealthcareOutput, ScenarioContext, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
from app.rag.retrieval_service import get_retrieval_service


async def run_healthcare(
...
) -> tuple[HealthcareOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "healthcare", n_results=2)
	...
	result = await call_agent(HEALTHCARE_PROMPT, user_content)
	try:
		output = HealthcareOutput.model_validate({**result, "agent_name": "healthcare"})
