...
from app.prompts import POLITICAL_PROMPT
from app.schemas import PoliticalOutput, ScenarioContext, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
from app.rag.retrieval_service import get_retrieval_service


async def run_political(
...
) -> tuple[PoliticalOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "politics", n_results=2)
	...
	result = await call_agent(POLITICAL_PROMPT, user_content)
	try:
		output = PoliticalOutput.model_validate({**result, "agent_name": "political"})
