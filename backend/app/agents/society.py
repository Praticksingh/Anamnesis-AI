...
from app.schemas import ScenarioContext, SocietyOutput, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
...
) -> tuple[SocietyOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "society", n_results=2)
