...
from app.schemas import ScenarioContext, TechnologyOutput, HistorianOutput, AgentOutputSummary, UnifiedTimelineEvent
from app.rag.retrieval_service import get_retrieval_service


async def run_technology(
...
) -> tuple[TechnologyOutput, list[str], list[str]]:
	retrieval_svc = get_retrieval_service()
	rag_context, retrieved_docs, sources = await retrieval_svc.retrieve(context, "technology", n_results=2)
