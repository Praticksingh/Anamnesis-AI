...
class FinalReport(Base):
	...
	causal_graph: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True, default=list)
	assumptions: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True, default=list)
	agent_confidences: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True, default=list)
	grounding_validations: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True, default=list)
	uncertainty_score: Mapped[float | None] = mapped_column(nullable=True, default=0.0)
	calibration_score: Mapped[int | None] = mapped_column(Integer, nullable=True, default=100)
	created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
